...
gradingStudents =map (\grade -> if failureGrade grade then grade else roundGrade grade 0)


failureGrade :: Integer -> Bool
failureGrade = (< 38)

roundGrade :: Integer -> Integer -> Integer
roundGrade grade diff
    | diff > 2 = grade
    | (grade + diff) `mod` 5 == 0 = grade + diff
    | otherwise = roundGrade grade (diff + 1)
